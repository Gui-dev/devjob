...
import { getJobApplicationsParamsSchema } from '@/http/validations/job-applications/get-job-applications-schema'
...
export const getJobApplicationsRoute = (app: FastifyInstance) => {
  app.withTypeProvider<ZodTypeProvider>().get(
    ...
    async (request, reply) => {
      ...
      const { jobApplications } = await getJobApplicationsUseCase.execute({
        jobId: job_id,
        userId,
      })

      return reply.status(200).send({ jobApplications })
    },
  )
}
